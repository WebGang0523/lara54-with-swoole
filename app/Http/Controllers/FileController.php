...
class FileController extends Controller
{
    public function uploadImage(Request $request)
    {
        ...
        $path = $image->storeAs('images/' . date('Y/m/d', $time), $filename);
        if ($path) {
            // 图片上传成功则将对应图片消息保存到 messages 表
            $message = new Message();
            $message->user_id = auth('api')->id();
            $message->room_id = $request->input('roomid');
            $message->msg = '';  // 文本消息留空
//            $message->img = Storage::disk('public')->url($path);
            $message->img = env('APP_URL').'/'.$path;
    ...
    public function avatar(Request $request)
    {
        ...
        $time = time();
        $filename = md5($time . mt_rand(0, 10000)) . '.' . $image->extension();
        $path = $image->storeAs('images/avatars/' . date('Y/m/d', $time), $filename);
        if ($path) {
            // 保存用户头像信息到数据库
            $user = auth('api')->user();
